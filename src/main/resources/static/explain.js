/* explain.js — a plain-English layer over this dashboard.
   ...
   in the portfolio; only this block is specific to the LLM gateway. */

window.EXPLAIN = {
  id: "llm-gateway",
  title: "The LLM gateway, explained",
  subtitle: "What this builds, and what every word on screen means",

  what:
    "This sits between an application and the AI providers it calls. It routes each request to " +
    "a provider, fails over to another when one breaks, keeps every customer inside their spend " +
    "budget, and — the interesting part — answers repeat questions from a cache instead of " +
    "paying for them twice.",

  why:
    "Calls to a language model are slow and billed per word. Two customers can ask the same " +
    "question in completely different words, so a normal cache that matches text exactly almost " +
    "never hits. Matching on *meaning* is what makes caching actually work here.",

  uses: [
    "Ask something in the Playground, then ask it again in different words and watch it hit the cache.",
    "Watch a provider go unhealthy and the request fail over to the next one in the chain.",
    "Spend a tenant's budget down and see the gateway refuse once it is exhausted.",
  ],

  honest:
    "Only a local echo provider is wired in, not a paid vendor — the failover seam is real and " +
    "tested, but an adapter that cannot be honestly tested without a paid key was left out on " +
    "purpose. The 90% cache hit rate is measured, not estimated.",

  steps: [
    {
      title: "Identify the caller",
      body:
        "An API key maps to a tenant, and the tenant has a token budget. Everything that follows " +
        "is charged against it.",
    },
    {
      title: "Look for a cached answer",
      body:
        "The question is turned into a vector — a list of numbers standing for its meaning — and " +
        "compared against previous questions. A close enough match is returned immediately, at no " +
        "cost to the provider.",
    },
    {
      title: "Otherwise call a provider",
      body:
        "The request goes to the first healthy provider in the failover chain. If it fails in a " +
        "way worth retrying, it is retried with a growing pause; if that provider keeps failing, " +
        "the breaker opens and it is skipped entirely.",
    },
    {
      title: "Charge the tokens and record it",
      body:
        "Tokens used are written to an append-only ledger and subtracted from the budget. Cache " +
        "hits are deliberately not charged, because they cost nothing upstream.",
    },
  ],

  glossary: {
    "semantic cache": {
      short: "A cache that matches questions by meaning rather than by exact wording.",
      why: "'How do I reset my password' and 'I forgot my login' want the same answer. An exact-match cache would miss that and pay twice.",
    },
    "cache": {
      short: "A store of previous answers, so the same work is not done twice.",
    },
    "hit rate": {
      short: "The share of requests answered from the cache instead of a provider.",
      why: "Measured at 90% here. Each hit is money not spent and an answer roughly 11× faster.",
    },
    "failover": {
      short: "Automatically moving to the next provider when the current one fails.",
      why: "Providers do go down. Without failover, their outage is your outage.",
    },
    "provider": {
      short: "An upstream AI service that actually answers the question.",
    },
    "breaker": {
      short: "A circuit breaker: after repeated failures from one provider, stop sending to it for a while.",
      why: "Retrying a dead provider wastes time on every single request. Skipping it is faster and kinder.",
    },
    "tenant": {
      short: "One customer of the gateway, with their own key, budget and usage history.",
    },
    "budget": {
      short: "How many tokens a tenant may spend before being refused.",
      why: "Language model calls cost real money. Without a hard limit, one runaway loop can spend a fortune overnight.",
    },
    "token": {
      short: "A chunk of text, roughly three quarters of a word. Providers bill per token.",
      why: "It is the unit everything is priced and budgeted in, which is why it is counted so carefully here.",
    },
    "ledger": {
      short: "An append-only record of every charge — entries are added, never edited or deleted.",
      why: "If usage records can be rewritten, a billing dispute cannot be settled from them.",
    },
    "retry": {
      short: "Trying a failed request again, with a pause that grows each attempt.",
      why: "Some failures are momentary. Retrying instantly and in lockstep just recreates the overload that caused them.",
    },
    "jitter": {
      short: "A small random amount added to each retry pause.",
      why: "Without it, every client that failed together retries together, and the stampede knocks the service over again.",
    },
    "streaming": {
      short: "Sending the answer word by word as it is generated, rather than waiting for all of it.",
      why: "It is why chat interfaces feel fast — you read the beginning while the end is still being written.",
    },
    "health": {
      short: "Whether a provider is currently answering successfully.",
    },
    "metrics": {
      short: "Counters and timings exported for monitoring — request rates, latencies, cache hits.",
    },
  },
};
/* ------------------------------------------------------------------------
